...
<body style="margin:0;padding:0;background-color:#f9f5f2;font-family:'Helvetica Neue',Helvetica,Arial,sans-serif;">
  ...
  private async send(to: string, subject: string, html: string) {
    try {
      ...
      // Tidak throw — jangan block main flow
    }
  }
  ...
  async sendOrderConfirmation(opts: {
    ...
    const paymentSection = opts.paymentUrl ? `
      <div style="background:#fff8f5;border:1px solid #f0d0c0;border-radius:8px;padding:16px 20px;margin:24px 0;">
        <p style="margin:0 0 8px;color:#b5533c;font-weight:600;font-size:14px;">⏳ Selesaikan Pembayaran</p>
    ...
    const content = `
      ...
      <table width="100%" cellpadding="0" cellspacing="0" style="margin-bottom:8px;">
        <thead>
          <tr>
            <th style="text-align:left;color:#888;font-size:12px;font-weight:600;padding-bottom:8px;text-transform:uppercase;letter-spacing:0.5px;">Produk</th>
            <th style="text-align:center;color:#888;font-size:12px;font-weight:600;padding-bottom:8px;text-transform:uppercase;letter-spacing:0.5px;">Qty</th>
            <th style="text-align:right;color:#888;font-size:12px;font-weight:600;padding-bottom:8px;text-transform:uppercase;letter-spacing:0.5px;">Harga</th>
          </tr>
        </thead>
      ...
      <table width="100%" cellpadding="0" cellspacing="0" style="margin-top:12px;">
        <tr>
          ...
          <td style="color:#333;font-size:14px;text-align:right;">Rp ${opts.subtotal.toLocaleString('id-ID')}</td>
        </tr>
        <tr>
          <td style="color:#555;font-size:14px;padding:4px 0;">Ongkos Kirim (${opts.courier} ${opts.service})</td>
          <td style="color:#333;font-size:14px;text-align:right;">Rp ${opts.shippingCost.toLocaleString('id-ID')}</td>
        </tr>
        <tr>
          <td style="color:#2d2d2d;font-size:16px;font-weight:700;padding:12px 0 4px;border-top:2px solid #f0e0d8;">Total</td>
          <td style="color:#b5533c;font-size:16px;font-weight:700;text-align:right;padding-top:12px;border-top:2px solid #f0e0d8;">Rp ${opts.total.toLocaleString('id-ID')}</td>
        </tr>
      </table>
    ...
    await this.send(opts.email, `🛍️ Pesanan ${opts.orderNumber} Diterima - MamaBear`, this.baseTemplate(content));
  }
    ...
    const content = `
      ...
      <div style="background:#fdf6f3;border-radius:8px;padding:20px;margin-bottom:24px;">
        <table width="100%" cellpadding="0" cellspacing="0">
          <tr>
            <td style="color:#888;font-size:12px;text-transform:uppercase;letter-spacing:0.5px;padding-bottom:4px;">Nomor Pesanan</td>
          </tr>
          <tr>
            <td style="color:#b5533c;font-size:18px;font-weight:700;padding-bottom:16px;">${opts.orderNumber}</td>
          </tr>
          <tr>
            <td style="color:#888;font-size:12px;text-transform:uppercase;letter-spacing:0.5px;padding-bottom:4px;">Kurir</td>
          </tr>
          <tr>
            <td style="color:#333;font-size:15px;font-weight:600;padding-bottom:16px;">${opts.courier} ${opts.service}</td>
          </tr>
          <tr>
            <td style="color:#888;font-size:12px;text-transform:uppercase;letter-spacing:0.5px;padding-bottom:4px;">Nomor Resi</td>
          </tr>
          <tr>
            <td style="color:#333;font-size:18px;font-weight:700;letter-spacing:1px;">${opts.trackingNumber}</td>
          </tr>
        </table>
      </div>
